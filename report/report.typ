...
- `scripts/` --- скрипт для множественного параллельного запуска бенчмарка
- `recommendation/` --- программа-рекомендатель
- `src/` --- файловые системы, генерация нагрузки, бенчмарк (основной код на `C++`)
- `ml_model/` --- исследование моделей и прочая работа по ML
- `report/` --- отчет
